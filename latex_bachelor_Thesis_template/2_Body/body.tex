...
P2 It is possible to switch to gear 5 and to reverse gear
P3 It is possible to switch gear in 1000 ms
P4 When the gearbox is in position N, th gear is not in position 1-5 or R
P5 The gear is never in N, when the gearbox is idle (expected to be neutral)
P6 In the case of no errors (  in ger and clutch) and ideal (engine) scenario,
a) a gear switch is guaranteerd in 900 ms (including 900 ms)
a') a gear switch is not guaranteerd in strictly less than 900 ms
b) it is impossible to switch gear in less than 150 ms
b') it is possible to switch gear at 150 ms
c) it is impossible to switch to gear in less than 400 ms if the switch is not from/to gear N
c') it is possible to switch gear at 400 ms if the switch is not from/to gear N
P7 When no errrors occcur  in gear and clutch), but engine fals to deliver zero torque:
a)  a gear switch is gearanteerd after 1055 ms ( not including  1055)
...
c) it is impossible to switch  gear in  less than 700 ms if the switch is not from/to gear N
c') it is posible to switch gear at 700 ms if the switch is not from/to gear N
...
p9 Clutcch errors
a)If the clutch is not closed properly (i.e. a timeout occurs) the gearbox  controller will enter the locationCCCloseError with 200 ms
b)  When the gearbox controller enters location CCloseError, there is always a problem in the clutch with closing the clutch.
a) If th clutch is not closed properly (ie. a timeout occurs) the gearbox controller will enter the location CCloseError within 200 ms
b) When the gearbox controller enters location CCloseError, there is always a problem in the clutch with closing the clutch.
p10 Gearbox errors
a) If the gearbox can not et a requested gear ( i.e. a tieout occurs) the gerbox controller will enter the location GsetError within 350 ms
b) When the gearbox controller enters location GSetError, there is always a problem in the gearbox with setting the gear.
p11 IF no error occurs in the engine, it is guaranteed to find synchronous spee
p12 When the gear is N, the engine is in initial or on its way to initial
p13 When the gear controller has a greater set, torque regulaton is always indicated in the engine
p14
a) If clutch is open, the gearbox controoller is in one of the predefined locations
b) If clutch is closed, the gearbox controller is in one of the predefined locations
p15
p16 If engine regulation on torque, then the clutch is closed
