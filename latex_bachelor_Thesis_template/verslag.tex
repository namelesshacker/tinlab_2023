...
%	delay actions (
	...
	\begin{enumerate}
		...
		\item fault tree analyse
		\item event tree analyse
...
%	provides experimental results and evaluates the method’s performance.
	...
	Concurrent systems are usually given by the text o a program or diagram for a circuit. Teere are may different types of concurrent systems (synchronous and asynchronous circuits, programs with shared variables, programs that communicate by message passing. And so on) 
...
Op deze manier is een systeem van een model testbaar op correctheid, rechability, safety,deadlock,responsiveness en liveness requirements. 
...
Formele verificatie is een systematich proces waarin bevestigd wordt of het bestudeerde systeem voldoet aan de specificaties of requirements  vastgesteld ine het specificatiedocument.
...
In de formele verificatieomgeving wordt ons mogelijk gemaak om te controleren of het op tijdautomaten gebaseerde model  met de temporele logische eigenschappen  voldoet aan dde gedefinieerde eigenschappen.    


Uppaal is een geïntegreerde toolomgeving voor het modelleren, simuleren en verifiëren van real-time  controllers en communicatieprotocollen,  , gezamenlijk ontwikkeld door Basic Research in Computer Science aan de Universiteit van Aalborg in Denemarken en de afdeling Informatietechnologie aan de Universiteit van Uppsala in Zweden.
